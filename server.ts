...
app.post("/api/chat", async (req, res) => {
  const { message, contextType, history } = req.body;
...
Current Context Mode: ${contextType || 'general'}`;
    ...
    const resultText = response.text || "I apologize, Counselor, but I am unable to formulate an answer due to an unexpected analytical error.";
    ...
    return res.json({
      text: `An error occurred while communicating with the AI core: ${error.message || error}. Displaying simulation answer for context: "${contextType}".\n\n` + (simulationAnswers[contextType as string] || simulationAnswers.general),
      citations: simulatedCitations,
      simulated: true,
      error: true
    });
